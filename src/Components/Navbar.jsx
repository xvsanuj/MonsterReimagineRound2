...
const Navbar = () => {
  ...
  useEffect(() => {
    h2sRef.current = document.querySelectorAll(".navLinks h2");
  }, []);
  ...
  return (
    <div className="h-12 lg:h-[10vh] justify-between flex items-center px-5 lg:px-20 w-full border-b-[1px] border-zinc-800 z-50">
        ...
        <div className="bg-black">
          <div className="bg-black mt-32 navLinks">
            {navli.map((item, index) => (
              <div key={index} className="bg-black overflow-hidden h-[9.5vh] w-fit">
                <h2 className="w-fit bg-black whitespace-nowrap mx-40 text-5xl my-5 font-semibold tracking-tighter cursor-pointer link">{item}</h2>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="lg:w-[25%] flex items-center gap-4">
        <h1 className="text-lg tracking-wide lg:text-3xl font-semibold lg:tracking-tight">Xiaomi</h1>
